...
func (s *Stack) FindRoute(id tcpip.NICID, localAddr, remoteAddr tcpip.Address, netProto tcpip.NetworkProtocolNumber, multicastLoop bool) (*Route, tcpip.Error) {
	...
	constructRouteWithLocalAddrFromAnyInterface := func(nic *nic, gateway tcpip.Address) *Route {
		// +checklocksignore: this is only called within `FindRoute`, and `s.mu` is locked throughout.
	...
	if r := func() *Route {
		...
		for _, route := range s.routeTable {
			...
			if onlyGlobalAddresses && chosenRoute.Equal(tcpip.Route{}) && isNICForwarding(nic, netProto) {
				...
				if r := constructRouteWithLocalAddrFromAnyInterface(nic, route.Gateway); r != nil {
					return r
				}
			}
		}
	...
	if !chosenRoute.Equal(tcpip.Route{}) {
		...
		if id == 0 {
			...
			if r := constructRouteWithLocalAddrFromAnyInterface(nic, gateway); r != nil {
				return r, nil
			}
		}
	}
