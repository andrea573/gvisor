...
// given labels, or fallen through if `mustHaveJumpedTo` is true.
// The fragment may not jump to any other label, nor return, nor fall through.
func (f syscallProgramFragment) mustHaveJumpedTo(allowFallthrough bool, labels ...label) {
	fragment := f.getFragment()
	outcomes := fragment.Outcomes()
	if outcomes.MayFallThrough && !allowFallthrough {
...
func (f syscallProgramFragment) MustHaveJumpedTo(labels ...label) {
	f.mustHaveJumpedTo(false /* allowFallthrough */, labels...)
}

// MustHaveJumpedTo asserts that the fragment must jump to one of the
// given labels, or fall through.
// The fragment may not jump to any other label, nor return.
func (f syscallProgramFragment) MustHaveJumpedToOrFallenThrough(labels ...label) {
	f.mustHaveJumpedTo(true /* allowFallthrough */, labels...)
}

// MayFallthrough returns whether the fragment may fall through.
func (f syscallProgramFragment) MayFallthrough() bool {
	return f.getFragment().Outcomes().MayFallThrough
}
