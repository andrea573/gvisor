...
func (fd *frontendFD) Ioctl(ctx context.Context, uio usermem.IO, sysno uintptr, args arch.SyscallArguments) (uintptr, error) {
	...
	// - Add handling below.
	handler, ok := fd.nvp.ioctlTable.frontendIoctl[nr]
	if !ok {
...
func rmControl(fi *frontendIoctlState) (uintptr, error) {
	...
	// - Add handling below.
	handler, ok := fi.fd.nvp.ioctlTable.controlCmd[ioctlParams.Cmd]
	if !ok {
...
func rmAllocR525(fi *frontendIoctlState) (uintptr, error) {
	return rmAlloc(fi, false /* isR535 */)
}

func rmAllocR535(fi *frontendIoctlState) (uintptr, error) {
	return rmAlloc(fi, true /* isR535 */)
}

func rmAlloc(fi *frontendIoctlState, isR535 bool) (uintptr, error) {
	var isNVOS64 bool
	if isR535 {
		switch fi.ioctlParamsSize {
		case nvgpu.SizeofNVOS21ParametersR535:
		case nvgpu.SizeofNVOS64ParametersR535:
			isNVOS64 = true
		default:
			return 0, linuxerr.EINVAL
		}
	...
	buf := nvgpu.GetRmAllocParamType(isNVOS64, isR535)
	if _, err := buf.CopyIn(fi.t, fi.ioctlParamsAddr); err != nil {
		return 0, err
	}
	ioctlParams := buf.ToOS64R535()
	...
	// - Add symbol and parameter type definitions to //pkg/abi/nvgpu.
	// - Add handling below.
	handler, ok := fi.fd.nvp.ioctlTable.allocationClass[ioctlParams.HClass]
	if !ok {
		fi.ctx.Warningf("nvproxy: unknown allocation class %#08x", ioctlParams.HClass)
		return 0, linuxerr.EINVAL
	}
	return handler(fi, &ioctlParams, isNVOS64, isR535)
}
...
func rmAllocSimple[Params any, PParams marshalPtr[Params]](fi *frontendIoctlState, ioctlParams *nvgpu.NVOS64ParametersR535, isNVOS64 bool, isR535 bool) (uintptr, error) {
	if ioctlParams.PAllocParms == 0 {
		return rmAllocInvoke[byte](fi, ioctlParams, nil, isNVOS64, isR535)
	}
	...
	n, err := rmAllocInvoke(fi, ioctlParams, &allocParams, isNVOS64, isR535)
	if err != nil {
		return n, err
	}
...
func rmAllocNoParams(fi *frontendIoctlState, ioctlParams *nvgpu.NVOS64ParametersR535, isNVOS64 bool, isR535 bool) (uintptr, error) {
	return rmAllocInvoke[byte](fi, ioctlParams, nil, isNVOS64, isR535)
}

func rmAllocEventOSEvent(fi *frontendIoctlState, ioctlParams *nvgpu.NVOS64ParametersR535, isNVOS64 bool, isR535 bool) (uintptr, error) {
	...
	n, err := rmAllocInvoke(fi, ioctlParams, &sentryAllocParams, isNVOS64, isR535)
	if err != nil {
		return n, err
	}
