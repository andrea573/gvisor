...
TEST(MountTest, ChangeMountFlags) {
  ...
  ASSERT_NO_ERRNO_AND_VALUE(Mount("", dir.path(), kTmpfs, 0, "mode=0700", 0));
  for (const auto& flag : flags) {
    struct statfs st;
    TEST_CHECK_SUCCESS(
        mount("", dir.path().c_str(), kTmpfs, MS_REMOUNT | flag, "mode=0700"));
    EXPECT_THAT(statfs(dir.path().c_str(), &st), SyscallSucceeds());
    EXPECT_EQ(st.f_flags, flag);
    // Resets mount flags.
    TEST_CHECK_SUCCESS(
        mount("", dir.path().c_str(), kTmpfs, MS_REMOUNT, "mode=0700"));
    ASSERT_THAT(statfs(dir.path().c_str(), &st), SyscallSucceeds());
    ASSERT_EQ(st.f_flags, 0);
  }
}

TEST(MountTest, ChangeBindMountFlags) {
  SKIP_IF(!ASSERT_NO_ERRNO_AND_VALUE(HaveCapability(CAP_SYS_ADMIN)));
  ...
  for (const auto& flag : flags) {
    struct statfs st;
    TEST_CHECK_SUCCESS(mount(dir1.path().c_str(), dir2.path().c_str(), kTmpfs,
                             MS_REMOUNT | MS_BIND | flag, "mode=0700"));
    EXPECT_THAT(statfs(dir2.path().c_str(), &st), SyscallSucceeds());
    EXPECT_EQ(st.f_flags, flag);
    // Resets mount flags.
    TEST_CHECK_SUCCESS(mount(dir1.path().c_str(), dir2.path().c_str(), kTmpfs,
                             MS_REMOUNT | MS_BIND, "mode=0700"));
